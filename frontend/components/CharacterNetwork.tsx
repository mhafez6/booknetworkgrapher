...
export default function CharacterNetwork({ nodes, edges }: Props) {
    ...
    useEffect(() => {
        ...
        // only show nodes that are connected
        ...
        //  only nodes that have edges, swap nodes for graphNodesRaw 
        const graphNodesRaw = nodes.filter((n) => connectedNames.has(n.name));
        ...
        const weightScale = d3.scaleLinear().domain([minWeight, maxWeight]).range([1, 8]);

        // Calculate node size based on mention count
        const minCount = Math.min(...nodes.map((n) => n.count));
        const maxCount = Math.max(...nodes.map((n) => n.count));
        const nodeScale = d3.scaleLinear().domain([minCount, maxCount]).range([8, 20]);

        // Prepare data
        const graphNodes: D3Node[] = graphNodesRaw.map((n) => ({
            id: n.name,
            count: n.count,
        }));
        ...
        simulation.on('tick', () => {
            link.attr('x1', (d) => (d.source as D3Node).x!)
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
                <span className="text-sm text-gray-600">
                    Hover over nodes to see character names • Drag to move • Scroll to zoom
                </span>
            </div>
